package com.github.theredbrain.redbrainstweaks.client.screen;
...
public class CookingPotScreen extends HandledScreen<CookingPotScreenHandler> {
    ...
    private static final Identifier BACKGROUND_TEXTURE = new Identifier(RedBrainsTweaks.MOD_ID, "textures/gui/cooking_pot.png");
    ...
    protected void renderMealDisplayTooltip(MatrixStack ms, int mouseX, int mouseY) {
        if (client != null && client.player != null && client.player.getInventory().getMainHandStack().isEmpty() && focusedSlot != null &&
            ...
            if (focusedSlot.id == 6) {
                ...
                tooltip.add(RedBrainsTweaks.i18n("container.cooking_pot.served_on", container).formatted(Formatting.GRAY));
