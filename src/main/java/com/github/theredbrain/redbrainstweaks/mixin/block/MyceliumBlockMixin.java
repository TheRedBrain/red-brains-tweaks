...
public class MyceliumBlockMixin {
    ...
    private static boolean canSurvive(BlockState state, WorldView world, BlockPos pos) {
        ...
        if (blockState.isOf(Blocks.SNOW) && (Integer)blockState.get(SnowBlock.LAYERS) == 1) {
            ...
            return false;
        } else if ((state.isOf(BlocksRegistry.LOOSE_DIRT_SLAB) || state.isOf(BlocksRegistry.DIRT_SLAB)) && state.get(Properties.WATERLOGGED)) {
            return false;
        } else {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!canSurvive(state, world, pos)) {
            ...
            if (world.getLightLevel(pos.up()) >= 9) {
                ...
                BlockState slabBlockState = BlocksRegistry.MYCELIUM_SLAB.getDefaultState();
                ...
                for(int i = 0; i < 4; ++i) {
                    ...
                    if (world.getBlockState(blockPos).isOf(Blocks.DIRT) && canSpread(fullBlockState, world, blockPos)) {
                        ...
                        world.setBlockState(blockPos, (BlockState)fullBlockState.with(SnowyBlock.SNOWY, world.getBlockState(blockPos.up()).isOf(Blocks.SNOW)));
                    } else if (world.getBlockState(blockPos).isOf(BlocksRegistry.DIRT_SLAB) && !(world.getBlockState(blockPos).get(Properties.WATERLOGGED)) && canSpread(slabBlockState, world, blockPos)) {
                    ...
                    } else if (world.getBlockState(blockPos).isOf(BlocksRegistry.LOOSE_DIRT_SLAB) && !(world.getBlockState(blockPos).get(Properties.WATERLOGGED)) && canSpread(slabBlockState, world, blockPos)) {
                        world.setBlockState(blockPos, (BlockState)slabBlockState.with(Properties.SLAB_TYPE, world.getBlockState(blockPos).get(Properties.SLAB_TYPE)).with(Properties.SNOWY, world.getBlockState(blockPos.up()).isOf(Blocks.SNOW) && world.getBlockState(blockPos).get(Properties.SLAB_TYPE) == SlabType.DOUBLE));
                    }
                }
            }
