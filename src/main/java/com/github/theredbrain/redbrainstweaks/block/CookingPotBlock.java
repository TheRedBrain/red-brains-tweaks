...
import com.github.theredbrain.redbrainstweaks.registry.SoundsRegistry;
...
public class CookingPotBlock extends BlockWithEntity implements InventoryProvider, Waterloggable {
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return EntitiesRegistry.COOKING_POT_ENTITY.instantiate(pos, state);
    }

    @Nullable
    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, EntitiesRegistry.COOKING_POT_ENTITY, CookingPotBlockEntity::tick);
    }
    ...
    public void appendTooltip(ItemStack stack, @Nullable BlockView world, List<Text> tooltip, TooltipContext options) {
        ...
        if (tag != null) {
            ...
            if (inventoryTag.contains("Items", 9)) {
                ...
                if (!meal.isEmpty()) {
                    MutableText servingsOf = meal.getCount() == 1
                            ? RedBrainsTweaks.i18n("tooltip.cooking_pot.single_serving")
                            : RedBrainsTweaks.i18n("tooltip.cooking_pot.many_servings", meal.getCount());
            ...
            tooltip.add(RedBrainsTweaks.i18n("tooltip.cooking_pot.empty").formatted(Formatting.GRAY));
        }
    }
