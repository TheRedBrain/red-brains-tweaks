...
public class BrownMushroomPlantBlock extends PlantBlock {
    ...
    public BrownMushroomPlantBlock(AbstractBlock.Settings settings, Supplier<RegistryEntry<? extends ConfiguredFeature<?, ?>>> feature) {
        super(settings);
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (random.nextInt(50) == 0 && world.getBaseLightLevel(pos, 0) < 1 && blockState.isOf(BlocksRegistry.RICH_SOIL)) {
            ...
            BlockState newState = BlocksRegistry.BROWN_MUSHROOM_COLONY.getDefaultState();
            if (world.getLightLevel(pos.up(), 0) <= 0 && newState.canPlaceAt(world, pos)) {
