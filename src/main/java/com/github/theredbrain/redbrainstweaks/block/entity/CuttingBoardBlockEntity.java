...
import com.github.theredbrain.redbrainstweaks.advancement.CuttingBoardTrigger;
import com.github.theredbrain.redbrainstweaks.block.CuttingBoardBlock;
import com.github.theredbrain.redbrainstweaks.inventory.*;
import com.github.theredbrain.redbrainstweaks.recipe.CuttingBoardRecipe;
import com.github.theredbrain.redbrainstweaks.registry.*;
import com.github.theredbrain.redbrainstweaks.tags.Tags;
import com.github.theredbrain.redbrainstweaks.util.CompoundTagUtils;
...
public class CuttingBoardBlockEntity extends BlockEntity {
    ...
    public CuttingBoardBlockEntity(BlockPos blockPos, BlockState blockState) {
        this(EntitiesRegistry.CUTTING_BOARD_ENTITY, blockPos, blockState, RecipeTypesRegistry.CUTTING_RECIPE_SERIALIZER.type());
    }
    ...
    public boolean processItemUsingTool(ItemStack tool, PlayerEntity player) {
        ...
        if (player != null) {
            if (recipeList.isEmpty()) {
                player.sendMessage(RedBrainsTweaks.i18n("block.cutting_board.invalid_item"), true);
            } else if (recipe == null) {
                player.sendMessage(RedBrainsTweaks.i18n("block.cutting_board.invalid_tool"), true);
            }
        }
