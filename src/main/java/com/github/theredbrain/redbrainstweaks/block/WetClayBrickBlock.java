...
public class WetClayBrickBlock extends AbstractBrickBlock {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if ((!world.getLevelProperties().isRaining() || world.getBiome(pos).isIn(RedBrainsTweaks.NO_PRECIPITATION)) && world.getLightLevel(LightType.SKY, pos.up()) >= 15 && random.nextInt(7) == 0) {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        return blockState.isOf(this) ? (BlockState)blockState.with(FACING, blockState.get(FACING)) : super.getPlacementState(ctx).with(FACING, ctx.getPlayerFacing());
    }
