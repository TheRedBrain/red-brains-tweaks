...
import com.github.theredbrain.redbrainstweaks.recipe.CookingPotRecipeSerializer;
import com.github.theredbrain.redbrainstweaks.recipe.CuttingBoardRecipe;
import com.github.theredbrain.redbrainstweaks.recipe.CuttingBoardRecipeSerializer;
...
public enum RecipeTypesRegistry {
    ...
    public static void registerAll() {
        for (RecipeTypesRegistry value : values()) {
            Registry.register(Registry.RECIPE_SERIALIZER, new Identifier(RedBrainsTweaks.MOD_ID, value.pathName), value.serializer());
            value.type = RecipeType.register(new Identifier(RedBrainsTweaks.MOD_ID, value.pathName).toString());
        }
    }
    ...
    private <T extends Recipe<? extends Inventory>> RecipeType<T> type(Class<T> clazz) {
        if (type == null) {
            type = RecipeType.register(new Identifier(RedBrainsTweaks.MOD_ID, pathName).toString());
        }
